...
use flappy::{ApplicationParameters, FlappyMessage, Operation, User, UserRole, LoginResult, InstantiationArgument, PracticeEntry, Tournament, TournamentStatus, TournamentResult};
...
impl Contract for FlappyContract {
    ...
    async fn instantiate(&mut self, args: Self::InstantiationArgument) {
        ...
        if let (Some(admin_username), Some(admin_hash)) = (args.admin_username, args.admin_hash) {
            ...
            self.state.users.insert(&admin_username, admin_user).expect("Failed to create admin user");
        }
    }
        ...
        match operation {
            ...
            Operation::LoginOrRegister { username, hash, requester_chain_id } => {
                ...
                if !*self.state.is_leaderboard_chain.get() {
                    ...
                    self.state.login_results.insert(&requester_chain_id, result).expect("Failed to store login result");
                    return;
                }
            ...
            Operation::CreateTournament { caller_chain_id, name, description, start_time, end_time } => {
                ...
                let admin_user = self.validate_admin_session(caller_chain_id).await
                ...
                self.state.tournaments.insert(&tournament_id, tournament).expect("Failed to create tournament");
            }

            Operation::JoinTournament { tournament_id, username } => {
                ...
                let mut tournament = self.state.tournaments.get(&tournament_id).await
            ...
            Operation::StartTournament { caller_chain_id, tournament_id } => {
                ...
                // Validate admin session
                let _admin_user = self.validate_admin_session(caller_chain_id).await
                    .unwrap_or_else(|msg| panic!("{}", msg));
            ...
            Operation::EndTournament { caller_chain_id, tournament_id, results } => {
            ...
            Operation::DeleteTournament { caller_chain_id, tournament_id } => {
                ...
                if tournament.status != TournamentStatus::Registration {
                    panic!("Can only delete tournaments in registration phase");
                }

                // Remove from tournaments map
                self.state.tournaments.remove(&tournament_id).expect("Failed to delete tournament");
            ...
            Operation::UpdateTournament { caller_chain_id, tournament_id, name, description, start_time, end_time } => {
            ...
            Operation::SubmitTournamentScore { tournament_id, username, score } => {
                ...
                if !*self.state.is_leaderboard_chain.get() {
                    ...
                    let mut my_scores = match self.state.my_tournament_scores.get(&tournament_id).await {
                        Ok(Some(scores)) => scores,
                        _ => Vec::new(),
                    };
                    my_scores.push(score);
                    self.state.my_tournament_scores.insert(&tournament_id, my_scores).expect("Failed to update personal tournament scores");
                    
                } else {
                    // Process directly on leaderboard chain
                    self.process_tournament_score(tournament_id, username, score).await;
                }
            }
        }
    }
    ...
    async fn execute_message(&mut self, message: Self::Message) {
        ...
        match message {
            ...
            // Tournament messages
            FlappyMessage::SubmitTournamentScore { tournament_id, username, score, player_chain_id: _ } => {
                // Only process on leaderboard chain
                if !*self.state.is_leaderboard_chain.get() {
                    return;
                }
            ...
            FlappyMessage::TournamentUpdate { tournament_id, tournament } => {
                // Update local tournament cache on player chains
                if !*self.state.is_leaderboard_chain.get() {
                    self.state.tournaments.insert(&tournament_id, tournament).expect("Failed to update tournament cache");
                }
            }

            FlappyMessage::RequestTournamentData { tournament_id, requester_chain_id } => {
                // Only process on leaderboard chain
                if !*self.state.is_leaderboard_chain.get() {
                    return;
                }
...
impl FlappyContract {
    ...
    async fn update_practice_leaderboard(&mut self) {
        ...
        let top_100: Vec<PracticeEntry> = all_practice_scores
            .into_iter()
            .take(100)
            .collect();
    ...
    async fn process_tournament_score(&mut self, tournament_id: String, username: String, score: u64) {
        ...
        self.update_tournament_leaderboard_with_score(&tournament_id, username, score).await;
    }

    async fn update_tournament_leaderboard_with_score(&mut self, tournament_id: &str, username: String, new_score: u64) {
        ...
        let existing_entry = leaderboard.iter_mut().find(|entry| entry.username == username);
        ...
        self.state.tournament_leaderboards.insert(tournament_id, leaderboard).expect("Failed to update tournament leaderboard");
    }

    async fn validate_admin_session(&self, caller_chain_id: ChainId) -> Result<User, String> {
        // Get login result for the caller's chain
        let login_result = self.state.login_results.get(&caller_chain_id).await
        ...
        let user = login_result.user.ok_or("No user in login result".to_string())?;
    ...
    async fn update_tournament_statuses(&mut self) {
        ...
        for tournament_id in tournament_ids {
            if let Ok(Some(mut tournament)) = self.state.tournaments.get(&tournament_id).await {
                ...
                if updated {
                    self.state.tournaments.insert(&tournament_id, tournament).expect("Failed to update tournament status");
                }
            }
        }
    }
